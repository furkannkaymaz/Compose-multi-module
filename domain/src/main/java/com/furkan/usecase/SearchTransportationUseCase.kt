...
class SearchTransportationUseCase @Inject constructor(
    ...
    fun searchTransportationByType(searchText: String): Flow<SearchTransportationUseCaseState> = flow {
            ...
            when (resource) {
                is Resource.Success -> {
                    ...
                    val filteredTransportation = transportationModelUi.transportation
                        ?.filter { transportation -> transportation.type.contains(searchText, ignoreCase = true) }
                        ?: emptyList()
                    if (filteredTransportation.isNotEmpty()) {
                        emit(SearchTransportationUseCaseState.TransportationDataSuccess(TransportationModelUi(filteredTransportation)))
                    } else {
                        emit(SearchTransportationUseCaseState.TransportationDataNotFound)
                    }
                }
                is Resource.Error -> {
    ...
    sealed class SearchTransportationUseCaseState {
        data class TransportationDataSuccess(val transportation: TransportationModelUi) : SearchTransportationUseCaseState()
        object TransportationDataNotFound : SearchTransportationUseCaseState()
